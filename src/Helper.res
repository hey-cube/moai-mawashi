...
let initParams = (): array<array<param>> => {
  ...
  let randomFloat = (min: float, max: float) => Random.float(max -. min) +. min

  let randomParam = (): param => {
    // 絶対値1以下と1以上が同じくらい生成されて欲しい
    let slope = Random.bool() ? randomFloat(-1., 1.) : randomFloat(-100., 100.)
    let cycle = randomFloat(30., diagonal)

    (slope, cycle)
  }

  let makeRow = (n: int) => Array.makeBy(n, _ => randomParam())
...
let calculateRotations = (intersection: point, mousePoint: point, params: array<param>): array<
  rotation,
> => {
